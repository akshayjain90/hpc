...
/*
This was the conventional deadlock case. Since the two sections were acquiring 
locks a and b in different order, when they run on parallel threads, they each acquire 
once lock and wait infinitely for the other.
This can be resolved easily by making all sections get locks in the same order. 
*/ 
