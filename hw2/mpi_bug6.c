...
if (rank < 2) {
  ...
  if (rank == 1) {
    src = 0;
    //offset whould be initialized to 0 as it will exceed the size of reqs array 
    //if initialized with REPS 
    //offset = REPS;
    offset = 0;
    }
  dest = src;
...
/* Task 2 does the blocking send operation */
  if (rank == 2) {
    //Since MPI_Send is blocking, it doesn't need wait_all. nreqs should be set to 0 for it.
    nreqs = 0;
    dest = 3;
    for (i=0; i<REPS; i++) {
