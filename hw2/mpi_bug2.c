...
//Integer should not be recieved in a float variable
//float beta;
int beta;
MPI_Request reqs[10];
MPI_Status stats[10];
...
if (rank == 1) {
  for (i=0; i<10; i++) {
    ...
    //An Integer is being sent by process 0. Recieving it in a MPI_FLOAT causes 
    //the program to interpret the sent integers as 0.0 which is wrong.
